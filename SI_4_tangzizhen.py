...
from tkinter import messagebox #导入messagebox用于弹窗提醒
import random #导入随机库
...
def draw_cell_by_cr(canvas, c, r, color="#CCCCCC"): #建立一个方法，在画板上绘制图像
    """
    ...
    :return:
    """
    x0 = c * cell_size #左上角的坐标
    y0 = r * cell_size
    x1 = c * cell_size + cell_size #右下角的坐标
...
win = tk.Tk()   #建立一个窗体
canvas = tk.Canvas(win, width=width, height=height, ) #新建一个画布对象
canvas.pack() #放置在指定位置

block_list = [] #记录已经生成的图案
for i in range(R):
...
def check_row_complete(row): #判断指定行是否满了
    for cell in row:
        if cell=='':
            return False
...
def check_and_clear():  #检查并且清除
    ...
    for ri in range(len(block_list)):
        if check_row_complete(block_list[ri]):
            ...
            global score  #每删除一行得十分
            score += 10

    if has_complete_row:  #重新绘制画板，并修改标题的得分
        draw_board(canvas, block_list)

        win.title("SCORES: %s" % score)


def save_block_to_list(block): #记录无法移动的俄罗斯方块
    shape_type = block['kind']
    cc, cr = block['cr']
    cell_list = block['cell_list']
...
def horizontal_move_block(event):#左右水平移动俄罗斯方块

    direction = [0, 0]  #keysym记录键的符号
...
def rotate_block(event): #变化俄罗斯方块
    global current_block
    ...
    block_after_rotate = {
        'kind': current_block['kind'],  # 旋转之后对应俄罗斯方块的类型
...
canvas.focus_set() # 聚焦到canvas画板对象上，把键盘上的键绑定到这个方法上
